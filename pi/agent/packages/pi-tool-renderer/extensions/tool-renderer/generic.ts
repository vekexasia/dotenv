...
function applyPatchChangesFromContext(context: any): ApplyPatchChange[] {
	if (Array.isArray(context?.state?._kendexApplyPatchChanges)) return context.state._kendexApplyPatchChanges as ApplyPatchChange[];
	try {
...
export function renderApplyPatchCall(args: any, theme: any, context: any): TruncatedLines | ReturnType<typeof makeEmpty> {
	...
	if (patchText) {
		try {
			...
			if (context?.argsComplete && context?.state) context.state._kendexApplyPatchChanges = changes;
		} catch {
