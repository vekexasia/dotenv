...
---
Use the registered `devIssuesInBatches` workflow for the requested GitHub/GitLab issues.
...
- If no issue list was provided, get only the numbers of open issues labeled `ready-for-agent` (for example, with `gh issue list --state open --label ready-for-agent --json number --jq '.[].number'`). The workflow fetches details and batches dependent issues.
