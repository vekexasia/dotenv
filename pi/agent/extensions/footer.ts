...
function formatModelLabel(modelId?: string): string {
  ...
  if (claudeMatch) {
    const family =
      claudeMatch[1]!.charAt(0).toUpperCase() +
      claudeMatch[1]!.slice(1).toLowerCase();
...
function buildThinkingDots(
  ...
  if (maxThinkingSupported && level === "max") {
    ...
    const dots = Array.from({ length: levels.length - 1 }, (_, index) =>
      index === highlight ? "✦" : "▪",
    ).join("");
  ...
  return (
    ...
    theme.fg("dim", "▫".repeat(levels.length - 1 - filled))
  );
}
...
function flag(
  ...
  enabled: boolean,
  char: string,
  color: "success" | "accent" | "warning" | "muted",
): string {
  ...
  ctx.ui.setFooter((tui, theme, footerData) => {
    ...
    return {
      ...
      render(width: number): string[] {
        ...
        const windowTokens = formatWindowTokens(
        ...
        const maxThinkingSupported =
          ctx.model?.reasoning === true &&
          ctx.model?.thinkingLevelMap?.max != null;
        ...
        const right = [
          ...
          theme.bold(model),
          " ",
          buildThinkingDots(pi, theme, maxThinkingSupported, maxAnimationFrame),
